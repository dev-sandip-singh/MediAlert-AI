...
export const getPatientDetailsController = errorHandleMiddleware(
  async (req, res, next) => {
    try {
      const patientId = req.params.patientId;  // Get the patientId from the URL
      console.log(patientId)
      const patient = await Appointment.findById(patientId);  // Fetch the patient from the database
      if (!patient) {
        return next(new ErrorHandler("Patient not found", 404));
      }
